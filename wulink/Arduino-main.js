/*
A simple editor that targets MicroArduino for the BBC micro:bit.
...
/*
Returns an object that defines the behaviour of the Arduino editor. The editor
...
function read(id)
{
        ...
        if(user==null){
           alert('请先登录');
          self.location='../user_login.php?url='+window.location.href;
		}
       $.ajax({
            type: "post",
            url: 'arduinoread.php',
...
function load(event) {
  ...
  var strFileName=filename.substring(0,filename.indexOf('.'));  //正则表达式获取文件名，不带后缀
  //$("#script-name").text(strFileName);
...
function ArduinoEditor(id) {
    ...
    ACE.getSession().setMode("ace/mode/Arduino");  // We're editing Arduino.
    ...
    // Sets the textual content of the editor (i.e. the Arduino script).
    ...
    editor.getSnippets = function() {
        ...
        return snippetManager.snippetMap.Arduino;
    };

    // Triggers a snippet by name in the editor.
    editor.triggerSnippet = function(snippet) {
        var snippetManager = ace.require("ace/snippets").snippetManager;
        var snippet = snippetManager.snippetNameMap.Arduino[snippet];
        if(snippet) {
    ...
    /*
    Turn a Arduino script into Intel HEX format to be concatenated at the
    end of the MicroArduino firmware.hex.  A simple header is added to the
    script.

    - takes a Arduino script as a string
    ...
    */
    ...
    // Generates a hex file containing the user's Arduino from the firmware.
    editor.getHexFile = function(firmware) {
        var hexlified_Arduino = this.hexlify(this.getCode());
        var insertion_point = ":::::::::::::::::::::::::::::::::::::::::::";
        return firmware.replace(insertion_point, hexlified_Arduino);
    }
    ...
    // Given an existing hex file, return the Arduino script contained therein.
...
function web_editor(config) {
 ...
 function doCouldsave() {
         tai=0;
        
		var blockly = $('#blockly');
		var xml = Blockly.Xml.workspaceToDom(Blockly.mainWorkspace);
		var tuxing = Blockly.Xml.domToText(xml);
        ...
        if(user==null){
          alert("请先登录");
          self.location='../user_login.php?url=../wulink';
		}
       $.ajax({
            type: "post",
            url: 'arduinochkname.php',
            async: false,
            data: JSON.stringify({
                user: user,
                sessionid: sessionid,
            ...
            success: function(msg) {
				...
				if($.trim(msg)==$.trim('1')){
					if(window.confirm('你有同名文件是否替换？')){}else{return false;}
				}
       ...
       $.ajax({
            ...
            data: JSON.stringify({
                ...
                tuxing:tuxing,
                //board:"arduino",
                codetext: output // 注意不要在此行增加逗号
            }),
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function(data) {
                alert(data);
				//$("#command-couldsave").removeAttr("disabled");
            } // 注意不要在此行增加逗号
        });
                ...
                $(vexContent).find('#load-form-form').on('submit', function(e){
                    ...
                    if(e.target[0].files.length === 1) {
                        ...
                        if (ext == 'py') {
                            ...
                            setDescription(config.translate.drop.Arduino);
...
//获取url中的参数
	...
	blockly.css('width', '98%');
            ...
            EDITOR.setCode(Blockly.Arduino.workspaceToCode(workspace));
        };
		...
		blockly.css('width', '98%');
                 ...
                 if(shouji=="pc")
	        {
                var zoomLevel = (fontSteps * zoomScaleSteps) + 1.0;
                                   ...
                                   var zoomLevel = (fontSteps * zoomScaleSteps) + 2.0;
                                }
    ...
    function doSnippets() {
        ...
        var context = {
            ...
            'snippets': snippetManager.snippetMap.Arduino,
            'describe': function() {
    ...
    function doAddnew() {
        // Triggered when the user wants to generate a link to share their code.
        var template = $('#addnew-template').html();
        Mustache.parse(template);
        vex.open({
            content: Mustache.render(template, config.translate.addnew)
        })
        $('#hint').focus();
        $('#button-decrypt-link').click(function() {
        var user=getCookie("username");
       ...
       $.ajax({
            ...
            success: function(msg) {
				...
				if($.trim(msg)==$.trim('errmac')){
					if(window.confirm('这是一个非法设备')){}else{return false;}
				}
				if($.trim(msg)==$.trim('erruser')){
					if(window.confirm('该设备已有绑定用户')){}else{return false;}
				}
				if($.trim(msg)==$.trim('olduser')){
					if(window.confirm('你已绑定该设备，请不要重复绑定！')){}else{return false;}
				}
				if($.trim(msg)==$.trim('ok')){
					$('#mac').append("<option value=\""+mac+"\">"+mac+"</option>");
					$testindex=$('#mac').children().length;

                    $('#mac option').eq($testindex-1).attr("selected",true);
					if(window.confirm('设备绑定成功')){}else{return false;}
					vex.close();
				}
    ...
    function doDel() {
		...
		var mac = $("#mac").val();
        var sessionid=getCookie("sessionid");
        var delmac='确定要删除设备'+mac;
		if(window.confirm(delmac)){}else{return false;}


       $.ajax({
            type: "post",
            url: 'arduinodelmac.php',
            async: false,
            data: JSON.stringify({
            ...
            success: function(msg) {
				if($.trim(msg)==$.trim('ok')){
					var checkIndex=$('#mac').get(0).selectedIndex;

					$('#mac option').eq(checkIndex).remove();
    ...
    function setupButtons() {
        $("#command-couldsave").click(function () {
            doCouldsave();
        });
        $("#command-couldnew").click(function () {
            window.location.href = "http://www.haohaodada.com/wulink/";
        });
        $("#tuxing-couldsave").click(function () {
            //dotuDownload();
            initBlockly();
        });
        $("#command-localsave").click(function () {
            dotuDownload();
           
        });
        $("#command-download").click(function () {

				alert("您将下载程序，编译时间为5-10秒，请稍候");
            if(tai==0){

				tai=1;
				doDownload();

				//setTimeout(cleartai,5000);

              }
        });
        $("#command-download1").click(function () {
            doDownload();
        });
		...
		$("#tuxing-load").click(function () {
			
            dotuDownload();
			//loadInput.click();
        });
		$("#tuxing-upload").click(function () {
