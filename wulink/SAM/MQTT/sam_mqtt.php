...
//var $debug = true;
  ...
  */
  ...
  function Receive($sub_id, $options=array()) {
    ...
    if ($this->debug) x("SAMConnection_MQTT.Receive() rc=$payload");
    return $rc;
  }
  ...
  function Send($topic, $message, $options=array()) {
    ...
    $msg = $this->fixed_header("MQTT_PUBLISH", 0, $qos,1) . $this->remaining_length(strlen($variable)+strlen($payload)) . $variable . $payload;
